...
const ninetyNinePlusPokeNames = pokémon.filter((p) => p.id > 99).map((p) => p.name) 
console.log(`\n\nAn array with just the names of pokémon with an id greater than 99: \n`)
console.log(ninetyNinePlusPokeNames)

const poisonPokemon = pokémon.filter((p) => p.types.includes("poison") && p.types.length === 1).map((p) => p.name)
...
const numOfNormalPokemon = pokémon.filter((p) => p.types.includes('normal')).length
